...
sum(ht1ec_totales+ht2ec_totales+hl1ec_totales+hl2ec_totales+hej1ec_totales+hej2ec_totales) TI,
sum(ht1c_totales+ht2c_totales+hl1c_totales+hl2c_totales+hej1c_totales+hej2c_totales+ht1ec_totales+ht2ec_totales+hl1ec_totales+hl2ec_totales+hej1ec_totales+hej2ec_totales) TT,
codigo,materia3  FROM `horas_asignatura` h inner join asignaturas a on h.cod_dpto=a.cod_dpto WHERE curso='#anio#' and materia3='#materia3#' group by materia3";
...
sum(ht1ec_totales+ht2ec_totales+hl1ec_totales+hl2ec_totales+hej1ec_totales+hej2ec_totales) TI,
sum(ht1c_totales+ht2c_totales+hl1c_totales+hl2c_totales+hej1c_totales+hej2c_totales+ht1ec_totales+ht2ec_totales+hl1ec_totales+hl2ec_totales+hej1ec_totales+hej2ec_totales) TT,
codigo,materia3  FROM `horas_asignatura` h INNER JOIN asignaturas a 
ON h.cod_dpto=a.cod_dpto WHERE curso='#anio#' and materia3='#materia3#' 
GROUP BY h.cod_asig 
ORDER BY materia3,TT DESC
";
...
sum(hl1ec_totales+hl2ec_totales+hej1ec_totales+hej2ec_totales) GPI,
sum(ht1ec_totales+ht2ec_totales+hl1ec_totales+hl2ec_totales+hej1ec_totales+hej2ec_totales) TI,
materia3  FROM `horas_docencia` h inner join asignaturas a on h.cod_dpto=a.cod_dpto 
WHERE curso='#anio#' and materia3='#materia3#' and nif='#nif#' group by nif";
...
sum(ht1ec_totales+ht2ec_totales+hl1ec_totales+hl2ec_totales+hej1ec_totales+hej2ec_totales) TI,
materia3  FROM `horas_docencia` h inner join asignaturas a on h.cod_dpto=a.cod_dpto 
WHERE curso='#anio#' and materia3='#materia3#'  group by materia3";
//comienzo del informe MAIN main

$sql[1]=$sqli[1];
$sql[1]=str_replace("materia3",$campomateria,$sql[1]); 

$result[1]=mysql_query($sql[1],$link); //Obtiene el nombre de todas las materias

while($row[1]=mysql_fetch_array($result[1])) //Para cada materia...
    {
    begintable();
        
    $sql[5]=str_replace("#materia3#",$row[1][$campomateria],$sqli[5]);
      $sql[5]=str_replace("#anio#",$anio,$sql[5]);
      $sql[5]=str_replace("materia3",$campomateria,$sql[5]);
	//echo $sql[5]."<br>"."<br>";
      $result[5]=mysql_query($sql[5],$link);
    
//obtiene cod_asig, GGE, GPE, TE, GGI, GPI, TI, TT, codigo, materia3, de las tablas horas_asignaturas join asignaturas para esta materia y el curso solicitado
    
    $row[5]=mysql_fetch_array($result[5]);

    
    $sql[4]=str_replace("#materia3#",$row[1][$campomateria],$sqli[4]);
	  $sql[4]=str_replace("#anio#",$anio,$sql[4]);
	  $sql[4]=str_replace("h.nif,","",$sql[4]);
	  $sql[4]=str_replace("materia3",$campomateria,$sql[4]);
	//echo $sql[4]."<br>"."<br>";
	  $result[4]=mysql_query($sql[4],$link);
	  $row[4]=mysql_fetch_array($result[4]);
	  $nombre_campomateria="nombre_$campomateria";
    escribefilascolumnas(1,$row[1][$campomateria],4,$row[1][$nombre_campomateria],1,"GGE",1,"GPE",1,"TE",1,"GGI",1,"GPI",1,"TI",1,"TT",1,"CSM%",1,"CT h",1,"CT%");
    escribefilascolumnas(1,$row[1][$campomateria],4,"Materia Total",1,$row[5]['GGE'],1,$row[5]['GPE'],1,$row[5]['TE'],1,$row[5]['GGI'],1,$row[5]['GPI'],1,$row[5]['TI'],1,$row[5]['TI']+$row[5]['TE'],
	      1,"",
	      1,"",
	      1,"");
     
     escribefilascolumnas(1,$row[1][$campo_materia],
    ...
    $sql[2]=str_replace("#materia3#",$row[1][$campomateria],$sqli[2]);
    $sql[2]=str_replace("materia3",$campomateria,$sql[2]);
	//echo $sql[2]."<br>"."<br>";
    $result[2]=mysql_query($sql[2],$link);
        
    escribefilascolumnas(1,$row[1][$campomateria],4,"Asignaturas",1,"GGE",1,"GPE",1,"TE",1,"GGI",1,"GPI",1,"TI",1,"TT",1,"CSM%",1,"CT h",1,"CT%");
    
    $sql[6]=str_replace("#materia3#",$row[1][$campomateria],$sqli[6]);
    $sql[6]=str_replace("#anio#",$anio,$sql[6]);
    $sql[6]=str_replace("#cod_dpto#",$row[2]['cod_dpto'],$sql[6]);
    $sql[6]=str_replace("materia3",$campomateria,$sql[6]);
    $result[6]=mysql_query($sql[6],$link);
    //bucle ASIGNATURAS    
    while($row[6]=mysql_fetch_array($result[6])){  
    
	  $sql[3]=str_replace("#materia3#",$row[1][$campomateria],$sqli[3]);
	  $sql[3]=str_replace("#anio#",$anio,$sql[3]);
	  $sql[3]=str_replace("materia3",$campomateria,$sql[3]);
		//echo sql[3];
	  $result[3]=mysql_query($sql[3],$link);

	  escribefilascolumnas(1,$row[1][$campomateria],1,"ASIG",
			      1,$row[6]['codigo'],
...
if (!isset($_GET['noprof']))
	{
	  escribefilascolumnas(1,$row[1][$campomateria],4,"Profesores.",1,"GGE",1,"GPE",1,"TE",1,"GGI",1,"GPI",1,"TI",1,"TT",1,"CSM%",1,"CT h",1,"CT%"); 
	    
	  escribefilascolumnas(1,$row[1][$campomateria],
			      4,"Profesores Total",
...
//Bucle Profesores
	  while($row[3]=mysql_fetch_array($result[3]))
		  ...
		  $nif=$row[3]['nif'];
		    ...
		    $TT=$row[3]['TT'];
		    $carga_sobre_materia=round($TT/$cargav*100,2);
		    $carga_sobre_maxima=round($TT/$cargamax*100,2);
		    escribefilascolumnas(1,$row[1][$campomateria],1,"PROF",
					1,$nif,1,$row[3]['cargo'],1,$row[3]['apellidos']."&#44; ".$row[3]['nombre'],
					1,$row[3]['GGE'],1,$row[3]['GPE'],1,$row[3]['TE'],1,$row[3]['GGI'],
					1,$row[3]['GPI'],1,$row[3]['TI'],
					1,$TT,
					1,$carga_sobre_materia,
					1,$cargav,
					1,$carga_real
					);  
